...
interface TermState {
  "RequiredTerms-1": boolean;
  "RequiredTerms-2": boolean;
  "OptionalTerms-1": boolean;
  "OptionalTerms-2": boolean;
}

export default function TermPage() {
  const [termsState, setTermsState] = useState<TermState>({
  ...
  const requiredTerms: (keyof TermState)[] = [
    "RequiredTerms-1",
    "RequiredTerms-2",
  ];
  const allTerms: (keyof TermState)[] = [
    "RequiredTerms-1",
    "RequiredTerms-2",
    "OptionalTerms-1",
    "OptionalTerms-2",
  ...
  const handleCheckboxChange = (termKey: keyof TermState) => {
  ...
  return (
    <div className="flex h-screen w-full justify-center bg-gray-100">
      <div className="w-[360px] bg-white px-5 py-6">
        {/* 뒤로가기 버튼 */}
        <button onClick={handleBack} className="mb-4 size-6">
          <img src="/ic_back.png" alt="뒤로가기" />
        </button>

        {/* 제목 */}
        <h1 className="mb-6 text-[25px] font-semibold text-[#8728FF]">
          약관 동의
        </h1>
        ...
        <button
          type="button"
          className={`mt-8 w-full rounded-md py-3 text-lg font-semibold text-white ${
